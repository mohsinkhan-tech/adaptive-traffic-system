...
with tab3:
    ...
    if not st.session_state.trained:
        ...
        if not st.session_state.trained:
            st.warning("⚠️ Pehle sidebar se **Train All Models** button click karo!")
            st.stop()

        ml_pred  = int(st.session_state.ml_model.predict(X_live)[0])
        ann_pred = int(st.session_state.ann_model.predict(X_live)[0])
        ens_pred = int(st.session_state.ensemble.predict(X_live)[0])
